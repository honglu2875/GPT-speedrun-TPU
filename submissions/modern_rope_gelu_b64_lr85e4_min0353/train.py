...
def _parse_experiment_profile(
    ...
    model = _config_keys(
        ...
        {
            "layers", "heads", "d_model", "mlp_mult", "mlp_hidden",
    ...
    result = ExperimentProfile(
        ...
        mlp_hidden=_config_int(
            model["mlp_hidden"], f"{prefix}.model.mlp_hidden", minimum=1
        ),
    ...
    if result.train_tokens is not None and result.train_tokens % result.seq_len:
        raise ValueError(
...
def resolve_config(
    ...
    if requested_train_tokens is not None:
        if requested_train_tokens % seq_len:
            raise ValueError(
                "training token budget must be divisible by seq_len "
                f"({seq_len:,}); got {requested_train_tokens:,}"
            )
        steps = math.ceil(requested_train_tokens / tokens_per_step)
        train_tokens = requested_train_tokens
    else:
    ...
    return Config(
        ...
        mlp_hidden=experiment.mlp_hidden,
...
def rms_norm(x: jax.Array, scale: jax.Array, dtype: Any) -> jax.Array:
    """RMS normalization used by the modernized candidate block."""
...
def tokens_after_step(step: int, config: Config) -> int:
    """Return exact cumulative tokens, capping a possible final partial batch."""

    return min(step * config.batch_size * config.seq_len, config.train_tokens)


def global_batch_for_step(step: int, config: Config) -> int:
    """Return the full batch except for a token-budget-closing final step."""

    previous_tokens = (step - 1) * config.batch_size * config.seq_len
    remaining_sequences = (config.train_tokens - previous_tokens) // config.seq_len
    return min(config.batch_size, remaining_sequences)
...
def write_training_csv(
    ...
    with temporary.open("w", encoding="utf-8", newline="") as handle:
        ...
        for index, (loss, learning_rate_value, grad_norm) in enumerate(history, 1):
            tokens_processed = tokens_after_step(index, config)
            writer.writerow(
                (
                    index,
                    tokens_processed,
...
def write_diagnostics_csv(
    ...
    with temporary.open("w", encoding="utf-8", newline="") as handle:
        ...
        for point in points:
            ...
            tokens_processed = tokens_after_step(point.step, config)
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    local_batch = local_batch_size(config.batch_size, process_count)
    final_global_batch = global_batch_for_step(config.steps, config)
    if final_global_batch % len(devices):
        raise ValueError(
            f"final global batch size {final_global_batch} must be divisible by "
            f"visible device count {len(devices)}"
        )
    final_local_batch = local_batch_size(final_global_batch, process_count)
    if config.attention_backend != "dense":
        console.phase(
    ...
    tokens_processed = config.train_tokens
    ...
    console.table(
        ...
        (
            ...
            (
                ...
                f"RoPE RMSNorm GELU-{config.mlp_hidden}",
            ),
            ("parameters", format_count(params_total)),
            ("global batch", f"{config.batch_size} × {config.seq_len} tokens"),
            *(
                (("final batch", f"{final_global_batch} × {config.seq_len} tokens"),)
                if final_global_batch != config.batch_size
                else ()
            ),
    ...
    final_sample_x = sample_x
    final_sample_y = sample_y
    if final_local_batch != local_batch:
        final_sample_x = put_host_local_array(
            np.zeros((final_local_batch, config.seq_len), dtype=np.int32),
            mesh,
            P("data", None),
            data_sharding,
            process_count,
        )
        final_sample_y = put_host_local_array(
            np.zeros((final_local_batch, config.seq_len), dtype=np.int32),
            mesh,
            P("data", None),
            data_sharding,
    ...
    if final_local_batch != local_batch:
        final_executable = compiled_step.lower(
            params, optimizer, final_sample_x, final_sample_y
        ).compile()
    train_compile_seconds = time.perf_counter() - compile_started

    diagnostic_executable: Any | None = None
    final_diagnostic_executable: Any | None = None
    ...
    if config.diagnostics_every:
        ...
        diagnostic_executable = compiled_diagnostic.lower(
            params, optimizer, sample_x, sample_y
        ).compile()
        final_diagnostic_executable = diagnostic_executable
        if final_local_batch != local_batch:
            final_diagnostic_executable = compiled_diagnostic.lower(
                params, optimizer, final_sample_x, final_sample_y
            ).compile()
    ...
    try:
        for step_index in range(1, config.steps + 1):
            ...
            with annotation:
                ...
                step_global_batch = global_batch_for_step(step_index, config)
                step_local_batch = local_batch_size(step_global_batch, process_count)
                partial_step = step_global_batch != config.batch_size
                ...
                if should_run_diagnostics(step_index, config):
                    diagnostic_runner = (
                        final_diagnostic_executable
                        if partial_step
                        else diagnostic_executable
                    )
                    if diagnostic_runner is None:  # defensive invariant
                        raise AssertionError("diagnostic executable was not compiled")
                    params, optimizer, last_metrics, diagnostic_values_at_step = (
                        diagnostic_runner(params, optimizer, batch_x, batch_y)
                    )
                    diagnostic_device_points.append(
                        (step_index, diagnostic_values_at_step)
                    )
                else:
                    step_executable = final_executable if partial_step else executable
                    params, optimizer, last_metrics = step_executable(
                ...
                if should_run_validation_probe(step_index, config):
                    ...
                    validation_rows.append(
                        ValidationRow(
                            step=step_index,
                            tokens_processed=(
                                tokens_after_step(step_index, config)
                            ),
                            kind="fineweb_probe",
                            domain="fineweb",
                ...
                if should_log:
                    ...
                    seen_tokens = tokens_after_step(step_index, config)
                    console.step(
                        step_index,
                        config.steps,
