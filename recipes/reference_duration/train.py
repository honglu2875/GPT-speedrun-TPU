...
def _parse_experiment_profile(
    ...
    duration_anchor_tpp = config_float(
        parameterization["base_tokens_per_parameter"],
        "family.parameterization.base_tokens_per_parameter",
    )
    ...
    if duration_anchor_tpp <= 0.0:
        raise ValueError(
            "config.yaml family.parameterization.base_tokens_per_parameter "
            "must be positive"
        )
