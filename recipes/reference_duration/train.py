...
class Config:
    ...
    def duration_multiplier(self) -> float:
        """Complete(d)P horizon factor relative to the configured TPP anchor."""

        if self.target_tokens_per_parameter is None:
            return 1.0
        return self.target_tokens_per_parameter / self.duration_anchor_tpp

    @property
    def data_multiplier(self) -> float:
        """Combined model-size and token-horizon data multiplier."""

        return self.ladder_data_multiplier * self.duration_multiplier
...
class ParameterizationDefinition:
    ...
    name: Literal["standard", "completedp_duration_v1"]
    base_tier: TierName
    duration_anchor_tpp: PositiveFloat
...
def resolve_config(
    ...
    *,
    ...
    duration_anchor_tpp = family.parameterization.duration_anchor_tpp
...
def init_params(config: Config, seed: int) -> dict[str, Any]:
    ...
    hidden_scale = (
        ...
        if config.parameterization == "completedp_duration_v1"
        else 0.02
    )
    ...
    return {
        ...
        "resolved": {
            ...
            "parameterization": {
                ...
                "ladder_data_multiplier": config.ladder_data_multiplier,
                "duration_anchor_tpp": config.duration_anchor_tpp,
                "duration_multiplier": config.duration_multiplier,
                "total_data_multiplier": config.data_multiplier,
