...
def model_console_rows(
    ...
    if not config.experts:
        return (
            (
                ...
                f"RoPE RMSNorm GELU bias-free MLP×{config.mlp_mult}",
            ),
    ...
    return (
        (
            "model",
            f"{config.tier} · L{config.layers} D{config.d_model} H{config.heads} · "
            f"bias-free MoE {config.experts}×top-{config.expert_top_k} "
...
def resolved_plan_metadata(config: Config) -> dict[str, Any]:
    ...
    return {
        ...
        "parameterization": config.parameterization,
        "weight_decay_policy": "weights_and_embeddings_only_v3_no_bias",
...
def active_parameter_count(params: Any, config: Config) -> int:
    ...
    per_expert = width * hidden + hidden * width
...
def expected_active_parameters(config: Config) -> int:
    ...
    The inherited tier denominator describes the biasful dense reference.
    This fork removes its QKV, attention-output, MLP-up, and MLP-down biases,
    preserves the dense MLP's active weight width exactly, and adds only the
    router projection. Keeping each term explicit makes this a real invariant
    rather than a tolerance around the declared tier.
    """

    declared = config.declared_parameters
    if declared is None or not config.experts:
        return declared
    reference_biases = (config.mlp_mult + 5) * config.d_model
    router = config.d_model * config.experts
    return declared + config.layers * (router - reference_biases)
...
def gpt_hidden(
    ...
    for block in params["blocks"]:
        ...
        qkv = linear(x_norm, block["qkv_w"], None, dtype)
        ...
        x = residual + (config.depth_multiplier ** (-config.depth_alpha)) * linear(
            attended, block["attn_w"], None, dtype
        )
        ...
        if config.experts:
            ...
            hidden = linear(x_norm, block["mlp_up_w"], None, dtype)
            hidden = jax.nn.gelu(hidden, approximate=True)
            mlp_out = linear(hidden, block["mlp_down_w"], None, dtype)
...
def weight_decay_mask(params: Any) -> Any:
    """Decay weights and embeddings, and reject any accidental bias parameter."""
    ...
    def decay_for_path(path: tuple[Any, ...], _value: Any) -> bool:
        ...
        if name.endswith("_scale"):
            return False
        if name.endswith(("_b", "_bias")):
            raise ValueError(
                f"bias parameter {name!r} is forbidden in the no-bias recipe"
            )
...
def run(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    result: dict[str, Any] = {
        ...
        "contract": {
            "model_id": "no-bias-moe-gpt-v1-family",
